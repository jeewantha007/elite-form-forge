          ...
          <div className="bg-gym-dark border border-primary-foreground/10 p-10">
            ...
            <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); handleLogin(); }}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-gym-black border border-primary-foreground/10 pl-11 pr-4 py-3 text-primary-foreground text-sm focus:border-primary focus:outline-none transition-colors" placeholder="your@email.com" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-gym-black border border-primary-foreground/10 pl-11 pr-4 py-3 text-primary-foreground text-sm focus:border-primary focus:outline-none transition-colors" placeholder="••••••••" />
                </div>
              </div>

              {error && <p className="text-xs text-status-overdue">{error}</p>}

              <button type="submit" className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground py-4 font-heading text-sm tracking-wider hover:bg-primary/90 transition-colors group">
            ...
            <div className="mt-6 border-t border-primary-foreground/10 pt-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-heading tracking-widest text-primary-foreground/40">ADMIN CREDENTIALS</span>
              </div>
              <div className="bg-gym-black border border-primary-foreground/5 p-3 space-y-1">
                <p className="text-xs text-primary-foreground/50">Email: <span className="text-primary-foreground/80">admin@powerzone.lk</span></p>
                <p className="text-xs text-primary-foreground/50">Password: <span className="text-primary-foreground/80">admin123</span></p>
              </div>
            </div>
          </div>
        </motion.div>
